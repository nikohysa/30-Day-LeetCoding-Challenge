 ...
 * Hints
 *
 * In-place means we should not be allocating any space for an extra array, but we are allowed to modify the existing array.
...
const moveZeroes = (nums) => {
    ...
    for (let index = 0; index < nums.length; index++) {
        if (nums[index] !== 0) {
            nums[lastNonZeroPosition++] = nums[index];
        }
    }

    for (let index = lastNonZeroPosition; index < nums.length; index++) {
        nums[index] = 0;
    }
};
